...
@Mod.EventBusSubscriber(modid = "statusbot", bus = Mod.EventBusSubscriber.Bus.MOD)
@Mod("statusbot")
public class StatusbotMain {
    ...
    public static StatusbotMain INSTANCE = null;
    ...
    @SubscribeEvent
    static void start(final ServerStartedEvent event) {
        ...
        BotManager.regBot(
                ...
                Parser.createStatusMessage(()-> Arrays.asList(INSTANCE.server.getPlayerNames()),INSTANCE.server.getPlayerNames().length),
                INSTANCE
        );
    }

    @SubscribeEvent
    static void stop(final ServerStoppingEvent event){
        ...
        if(BotManager.jda!=null){
            ...
            EmbedManager.tryUpdateAllEmbeds(INSTANCE);
        }
        DataManager.saveAllData(INSTANCE);
        try {
            Thread.sleep(1000);
    ...
    @SubscribeEvent
    static void joined(final PlayerEvent.PlayerLoggedInEvent event) {
        BotManager.regBot(
                ConfigManager.configuration.getString("bot_token"),
                Parser.createStatusMessage(()->Arrays.asList(INSTANCE.server.getPlayerNames()),INSTANCE.server.getPlayerNames().length),
                INSTANCE
        ...
        if(BotManager.jda!=null) {
            if (ConfigManager.getBool("enable_server_join_messages")) {
                String startMessage = Parser.createJoinMessage(
                        ()->Arrays.asList(INSTANCE.server.getPlayerNames()),
                        event.getEntity().getName().getString(),
                        INSTANCE.server.getPlayerNames().length
                );
                if (ConfigManager.getBool("enable_text_channel_status_messages")) {
                    for (long id : BotManager.messageTextChannels) {
                        TextChannel channel = BotManager.jda.getTextChannelById(id);
    ...
    @SubscribeEvent
    static void left(final PlayerEvent.PlayerLoggedOutEvent event) {
        BotManager.regBot(
                ConfigManager.configuration.getString("bot_token"),
                Parser.createStatusMessage(()->makeStringList(INSTANCE.server.getPlayerNames(), event.getEntity().getName().getString()),INSTANCE.server.getPlayerNames().length-1),
                INSTANCE
        );
        if(BotManager.jda!=null) {
            if (ConfigManager.getBool("enable_server_leave_messages")) {
                String startMessage = Parser.createLeaveMessage(
                        ()->makeStringList(INSTANCE.server.getPlayerNames(),event.getEntity().getName().getString()),
                        event.getEntity().getName().getString(),
                        INSTANCE.server.getPlayerNames().length-1
                );
                if (ConfigManager.getBool("enable_text_channel_status_messages")) {
                    for (long id : BotManager.messageTextChannels) {
                        TextChannel channel = BotManager.jda.getTextChannelById(id);
    ...
    static void tick(TickEvent.ServerTickEvent event){
        if (BotManager.jda!=null&&BotManager.jda.getStatus()== JDA.Status.CONNECTED) {
            if(INSTANCE.server.getTickCount()%200==0){
                EmbedManager.tryUpdateAllEmbeds(INSTANCE);
            }
        }
    }
